...
DEBUGGING_PATH = (
    ...
    if (raw_debugging_path := os.get("DEBUGGING_PATH", None))
    else None
)
if DEBUGGING_PATH:
...
def execute_command(account, command, args=None):
        ...
        for command in CHOICES:
            ...
            if DEBUGGING_PATH:
                with open(DEBUGGING_PATH / f"{command}.json", "w+") as out_command:
                    out_command.write(json.dumps(execute_command(account, command)))
        return
    ...
    for network in account["networks"]["data"]:
        ...
        if command == "reboot":
            ...
            result = eero.__getattribute__(command)(
...
if __name__ == "__main__":
    if eero.needs_login():
        ...
        parser = ArgumentParser()
        parser.add_argument("command", choices=CHOICES, help="info to print")
